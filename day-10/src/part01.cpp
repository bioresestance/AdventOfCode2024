...
void checkRoute(std::vector<std::vector<uint8_t>> &map, uint8_t next_index, Coordinate coord_to_check, std::unordered_set<Coordinate> &found)
{
    ...
    if (next_index == 9)
    {
        found.emplace(coord_to_check);
        return;
    }

    // Check all four directions.
    checkRoute(map, next_index + 1, coord_to_check + Coordinate{0, -1}, found); // UP
    checkRoute(map, next_index + 1, coord_to_check + Coordinate{-1, 0}, found); // LEFT
    checkRoute(map, next_index + 1, coord_to_check + Coordinate{0, 1}, found);  // DOWN
    checkRoute(map, next_index + 1, coord_to_check + Coordinate{1, 0}, found);  // RIGHT
}
...
int64_t handlePart1(const std::vector<std::string> &inputLines)
{
    ...
    for (const auto [rowIdx, row] : map | enumerate)
    {
        for (const auto [colIdx, col] : row | enumerate)
        {
            ...
            {
                std::unordered_set<Coordinate> found;
                checkRoute(map, 0, Coordinate{static_cast<int>(colIdx), static_cast<int>(rowIdx)}, found);
                total += found.size();
            }
        }
    }
